...
(function(f) {
		...
		function E(ad) {
					...
					if (T.extraData) {
						for ( var an in T.extraData) {
							if (T.extraData.hasOwnProperty(an)) {
								if (f.isPlainObject(T.extraData[an])
										&& T.extraData[an]
												.hasOwnProperty("name")
										&& T.extraData[an]
			...
			function V(an) {
				...
				try {
					...
					var ak = T.dataType == "xml" || af.XMLDocument
							|| f.isXMLDoc(af);
					d("isXml=" + ak);
					if (!ak && window.opera
							&& (af.body === null || !af.body.innerHTML)) {
						if (--ac) {
					...
					if (aq) {
						M.status = Number(aq.getAttribute("status"))
								|| M.status;
						M.statusText = aq.getAttribute("statusText")
								|| M.statusText
					}
					...
					if (ao || T.textarea) {
						...
						if (am) {
							...
							M.status = Number(am.getAttribute("status"))
									|| M.status;
							M.statusText = am.getAttribute("statusText")
									|| M.statusText
						} else {
							if (ao) {
								...
								if (ai) {
									M.responseText = ai.textContent ? ai.textContent
											: ai.innerText
								} else {
									if (ar) {
										M.responseText = ar.textContent ? ar.textContent
												: ar.innerText
									}
								}
							}
				...
				if (M.status) {
					al = (M.status >= 200 && M.status < 300 || M.status === 304) ? "success"
							: "error"
				}
				if (al === "success") {
					if (T.success) {
			...
			var U = f.parseXML
					|| function(ah, ai) {
						if (window.ActiveXObject) {
							...
							ai = (new DOMParser()).parseFromString(ah,
									"text/xml")
						}
						return (ai && ai.documentElement && ai.documentElement.nodeName != "parsererror") ? ai
								: null
					};
			...
			var k = function(am, ak, aj) {
				var ai = am.getResponseHeader("content-type") || "", ah = ak === "xml"
						|| !ak && ai.indexOf("xml") >= 0, al = ah ? am.responseXML
						: am.responseText;
	...
	f.fn.ajaxForm = function(g) {
		...
		if (g.delegation) {
			f(document).off("submit.form-plugin", this.selector, b).off(
					"click.form-plugin", this.selector, a).on(
					"submit.form-plugin", this.selector, g, b).on(
					"click.form-plugin", this.selector, g, a);
			return this
		}
		return this.ajaxFormUnbind().bind("submit.form-plugin", g, b).bind(
